...
const AURA_SCENE := preload("res://scenes/combat/hitboxes/ZhalrethAura.tscn")
...
const BUFF_COLOR := Color(0.75, 0.15, 0.25)
...
func cast_skill(skill: SkillData, aim_position: Vector2 = Vector2.INF) -> bool:
	match skill.skill_type:
		...
		SkillData.SkillType.AOE_SLOW:
			_crippling_wave(skill, aim_position)
			return true
...
func _crippling_wave(skill: SkillData, aim_position: Vector2 = Vector2.INF) -> void:
...
func _bloodlust(_skill: SkillData) -> void:
	...
	if _buff_timer == null:
		...
		_buff_timer.timeout.connect(_end_bloodlust)
...
func _dark_aura(skill: SkillData) -> void:
